...
select concat(FirstName,' ', ifnull(MiddleName, ''),' ', LastName) as FullName, Salary from telerikacademy.employees;
...
select * from telerikacademy.employees e
...
select * from telerikacademy.employees e
join telerikacademy.employees m on e.ManagerID = m.EmployeeID
join telerikacademy.addresses a on e.AddressID = e.AddressID;
...
where d.Name in ("Sales", "Finance") and YEAR(HireDate) between 1995 and 2005;
